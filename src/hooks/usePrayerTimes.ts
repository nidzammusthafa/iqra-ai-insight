...
export const usePrayerTimes = () => {
  ...
  const getCoordinatesFromTimezone = (timezone: string) => {
    const timezoneCoords: {
      [key: string]: { lat: number; lng: number; city: string };
    } = {
      ...
      "Asia/Kuala_Lumpur": { lat: 3.139, lng: 101.6869, city: "Kuala Lumpur" },
  ...
  useEffect(() => {
    ...
    const fetchPrayerTimes = async () => {
      try {
        ...
        const times: PrayerTime[] = Object.entries(PRAYER_NAMES).map(
          ([key, value]) => {
  ...
  useEffect(() => {
    ...
    const updatedTimes = prayerTimes.map((prayer, index) => {
      ...
      const isInWindow =
        currentTimeMinutes >= prayerTimeMinutes &&
